...
<div className="relative overflow-hidden x-shadow-green feature-card p-5">
      ...
      <div className="w-full mx-auto bg-white rounded-lg">
        {user.map((values, index) => (
          <div key={index} className="flex flex-col items-center pb-10">
            ...
            <img src={userProfilePic} className='img-1' id='img-1' onClick={()=>
            {
            ...
            <img src="https://images.unsplash.com/photo-1682687982167-d7fb3ed8541d?ixlib=rb-4.0.3&ixid=M3wxMjA3fDF8MHxlZGl0b3JpYWwtZmVlZHw2fHx8ZW58MHx8fHx8&auto=format&fit=crop&w=600&q=60" className='img-2' id='img-2' onLoad={()=>{
            ...
            <span className="text-sm text-[#F44336] dark:text-gray-400 uppercase ">
              {values.userclass}
            </span>
      ...
      <div className='w-[75%] mx-auto'>
        <div className="grid md:grid-cols-2 gap-8 mt-6 justify-center">
          <div className="relative overflow-hidden x-shadow-yellow feature-card p-5 h-[150px] max-w-[18rem]">
            <h5 className="font-inter font-bold mb-2 text-3xl tracking-tight text-[#FFCA28] dark:text-white">
              YOUR TAGS
            </h5>
  ...
  {user.map((values, index) => (
    <div key={index} className="font-inter text-3xl text-black dark:text-gray-400">
  ...
  <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400 rounded-xl border-2 overflow-hidden">
    <thead className="text-xl text-[#FFCA28] uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
      <tr>
        <th
          scope="col"
          className="px-4 md:px-6 py-3 border-b dark:border-gray-700 md:text-lg border-r border-black"
        >
          Event name
        </th>
        <th
          scope="col"
          className="px-4 md:px-6 py-3 border-b dark:border-gray-700 md:text-lg flex justify-center items-center border-r border-black"
        >
          Preview
        </th>
        <th
          scope="col"
          className="px-4 md:px-6 py-3 border-b text-center border-r text-lg border-black"
        >
          Status
        </th>
        <th
          scope="col"
          className="px-4 md:px-6 py-3 border-b text-center text-lg border-black"
        >
          Tags
        </th>
      </tr>
    ...
    <tbody>
      {events.map((values, index) => (
        ...
        >
          <td
            scope="row"
            className="px-4 md:px-6 py-4 font-medium border-black text-[#4285F4] whitespace-nowrap dark:text-white md:text-lg border-r"
          >
            {values.eventname}
          </td>
          <td className="px-4 md:px-6 py-4 border-black dark:border-gray-700 text-lg md:flex md:justify-center md:items-center border-r">
            <Image
          ...
          <td className="px-4 md:px-6 py-4 border-b text-center border-black dark:border-gray-700 text-[#35A853] text-lg">
            {values.eventTags}
          </td>
        </tr>
      ))}
